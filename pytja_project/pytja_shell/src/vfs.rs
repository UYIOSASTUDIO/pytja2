...
impl VirtualFileSystem {
    ...
    async fn check_quota_availability(&self, size_needed: usize) -> Result<(), PytjaError> {
        let used = self.db().get_total_usage(&self.user_id).await.unwrap_or(0);
    ...
    pub async fn create(&mut self, mut name: String, is_folder: bool, content: Vec<u8>, system_override: bool, lock_pass: Option<String>) -> Result<String, PytjaError> {
        if !system_override {
            if !is_folder {
                ...
                if !has_valid_ext && !name.contains('.') {
                    name.push_str(".txt");
                }
            }
    ...
    pub async fn delete_all_inside(&mut self, target_path_opt: Option<&str>) -> Result<String, PytjaError> {
        ...
        if let Some(node) = self.db().get_node(&dir_path).await? {
            if node.owner != self.user_id {
                return Err(PytjaError::AccessDenied("Permission denied".to_string()));
            }
        }

        let items = self.db().list_directory(&dir_path).await?;
        if items.is_empty() { return Ok("Directory is already empty.".to_string()); }

        let mut count = 0;
        for item in items {
            if item.owner == self.user_id {
                self.db().delete_node_recursive(&item.path).await?;
                count += 1;
            }
        }
    ...
    pub async fn chmod(&mut self, name: &str, lock_pass: Option<String>) -> Result<String, PytjaError> {
        ...
        // Hier setzen wir nur das Passwort (Lock), Permission Level bleibt gleich (None Update)
        self.db().update_metadata(&full_path, lock_pass, None).await?;
    ...
    pub async fn chmod_permissions(&mut self, name: &str, level: u8) -> Result<String, PytjaError> {
        ...
        if node.owner != self.user_id {
            return Err(PytjaError::AccessDenied("Only owner can change permissions.".to_string()));
        }

        if level > 2 {
            return Err(PytjaError::System("Invalid permission level (0-2).".to_string()));
        }

        self.db().update_permissions(&full_path, level).await?;
    ...
    pub async fn chown(&mut self, name: &str, new_owner: &str) -> Result<String, PytjaError> {
        ...
        if node.owner != self.user_id {
            return Err(PytjaError::AccessDenied("Only owner can transfer file".to_string()));
        }
        self.db().update_metadata(&full_path, None, Some(new_owner.to_string())).await?;
    ...
    pub async fn edit_file(&mut self, name: &str) -> Result<String, PytjaError> {
        ...
        temp_file.write_all(&node.content)?;
        let temp_path = temp_file.path().to_str().unwrap().to_string();

        let status = std::process::Command::new("nano").arg(&temp_path).status()?;
        if !status.success() {
            return Err(PytjaError::System("Editor exited with error".to_string()));
        }
        ...
        if diff > 0 {
            let used = self.db().get_total_usage(&self.user_id).await.unwrap_or(0);
            if used + diff as usize > DEFAULT_QUOTA {
        ...
        self.db().save_node(&new_node).await?;
        let _ = self.db().log_action(&self.user_id, "EDIT", &full_path).await;
    ...
    pub async fn copy(&mut self, source: &str, dest: &str) -> Result<String, PytjaError> {
        ...
        let src_node = self.db().get_node(&old_path).await?
            .ok_or_else(|| PytjaError::NotFound("Source not found".to_string()))?;

        if src_node.is_folder {
            return Err(PytjaError::System("Folder copy not supported yet.".to_string()));
        }
        ...
        } else if let Ok(Some(node)) = self.db().get_node(&new_path_str).await {
            if node.is_folder {
                let src_name = Path::new(&old_path).file_name().unwrap().to_str().unwrap();
        ...
        if self.db().get_node(&new_path_str).await?.is_some() {
        ...
        new_node.created_at = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs_f64();
    ...
    pub async fn move_rename(&mut self, source: &str, dest: &str, lock_pass: Option<String>) -> Result<String, PytjaError> {
        ...
        let node = self.db().get_node(&old_path).await?
            .ok_or_else(|| PytjaError::NotFound("Source not found".to_string()))?;
        ...
        if let Ok(Some(dest_node)) = self.db().get_node(&new_path_str).await {
        ...
        self.db().move_path(&old_path, &new_path_str).await?;

        if let Some(pass) = lock_pass {
            self.db().update_metadata(&new_path_str, Some(pass), None).await?;
    ...
    pub async fn import_from_host(&mut self, host_path_str: &str, _target_vfs_path: Option<String>, lock_pass: Option<String>, recursive_lock: bool) -> Result<String, PytjaError> {
        ...
        if !host_path.exists() {
            return Err(PytjaError::NotFound("Host path not found".to_string()));
        }

        let name = host_path.file_name().ok_or(PytjaError::System("Invalid path".to_string()))?
        ...
        if host_path.is_dir() {
            ...
            match self.create(name.clone(), true, vec![], false, lock_pass.clone()).await {
                Ok(_) => {},
                Err(PytjaError::AlreadyExists(_)) => {}, // Ignorieren bei Ordnern
                Err(e) => return Err(e),
            }

            let target_root = if self.current_path == "/" {
                format!("/{}", name)
            } else {
                format!("{}/{}", self.current_path, name)
            };
    ...
    fn import_recursive<'a>(&'a mut self, host_path: std::path::PathBuf, vfs_parent: String, lock_pass: Option<String>, rec_lock: bool)
        ...
        Box::pin(async move {
            ...
            for entry in entries {
                ...
                let vfs_path = if vfs_parent == "/" {
                    format!("/{}", name)
                } else {
                    format!("{}/{}", vfs_parent, name)
                };
    ...
    fn print_tree_recursive(&self, path: String, prefix: String) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), PytjaError>> + Send + '_>> {
        Box::pin(async move {
            let items = self.db().list_directory(&path).await?;
            ...
            for (i, item) in sorted_items.iter().enumerate() {
                ...
                let name_display = if item.is_folder {
                    item.name.blue().bold()
                } else {
    ...
    fn check_access(&self, node: &FileNode, access: AccessType) -> Result<(), PytjaError> {
        // 1. Owner darf alles (Der Gott-Modus)
        if node.owner == self.user_id {
            return Ok(());
        }
        ...
        match access {
            AccessType::Read => {
                // Public Read (1) oder Public Write (2) erlaubt Lesen
                if node.permissions >= 1 {
                    return Ok(());
                }
            },
            AccessType::Write => {
                // Nur Public Write (2) erlaubt Schreiben für Fremde
                if node.permissions >= 2 {
                    return Ok(());
                }
            },
            _ => {}
