...
impl PytjaRepository for SqliteDriver {
    async fn init(&self) -> Result<(), PytjaError> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS audit_logs (
                ...
                user_id TEXT, -- Einheitlich user_id statt actor
                action TEXT,
                target TEXT,
                timestamp REAL
            );
        ...
        if count == 0 {
            ...
            sqlx::query("INSERT INTO roles (name, permissions) VALUES ('admin', ?)")
                .bind(admin_perms)
                .execute(&self.pool).await.ok();

            // Guest Rolle
            let guest_perms = serde_json::to_string(&vec!["core:fs:read"]).unwrap();
            sqlx::query("INSERT INTO roles (name, permissions) VALUES ('guest', ?)")
                .bind(guest_perms)
                .execute(&self.pool).await.ok();

            // User Rolle (Standard)
            let user_perms = serde_json::to_string(&vec!["core:fs:read", "core:fs:write", "core:exec"]).unwrap();
            sqlx::query("INSERT INTO roles (name, permissions) VALUES ('user', ?)")
                .bind(user_perms)
                .execute(&self.pool).await.ok();
        }

        info!("SQLite DB initialized (Enterprise Schema v2)");
        Ok(())
    }
    ...
    async fn list_users(&self) -> Result<Vec<User>, PytjaError> {
        // Manuelles Mapping für volle Kontrolle über fehlende Felder
    ...
    async fn save_user_keys(&self, _username: &str, _public_key: &[u8], _private_key_encrypted: &[u8]) -> Result<(), PytjaError> {
        // Keys werden bei create_user gespeichert.
        Ok(())
    }
    ...
    async fn save_node(&self, node: &FileNode) -> Result<(), PytjaError> {
        sqlx::query(
            "INSERT OR REPLACE INTO file_nodes (path, name, owner, is_folder, size, content, blob_id, lock_pass, permissions, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
            .bind(&node.path)
            .bind(&node.name)
    ...
    async fn list_directory(&self, path: &str) -> Result<Vec<FileNode>, PytjaError> {
        // Saubere Pfad-Logik für "Kinder dieses Ordners"
        ...
        for row in rows {
            // Filtern: Nur direkte Kinder (keine Enkel)
            let p: String = row.try_get("path").unwrap_or_default();
            // Wenn der Pfad noch einen Slash nach dem Prefix hat, ist es ein Enkel -> Skip
            ...
            nodes.push(FileNode {
                ...
                owner: row.try_get("owner").unwrap_or_default(),
                is_folder: row.try_get("is_folder").unwrap_or(false),
                content: vec![], // Content nicht laden bei List!
                blob_id: row.try_get("blob_id").ok(),
                size: row.try_get::<i64, _>("size").unwrap_or(0) as usize,
                lock_pass: row.try_get("lock_pass").ok(),
                permissions: row.try_get::<i32, _>("permissions").unwrap_or(0) as u8,
    ...
    async fn update_metadata(&self, path: &str, lock: Option<String>, owner: Option<String>) -> Result<(), PytjaError> {
        if let Some(l) = lock {
            sqlx::query("UPDATE file_nodes SET lock_pass = ? WHERE path = ?")
                .bind(l).bind(path)
                .execute(&self.pool).await.map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
        }
        if let Some(o) = owner {
            sqlx::query("UPDATE file_nodes SET owner = ? WHERE path = ?")
                .bind(o).bind(path)
                .execute(&self.pool).await.map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
        }
        Ok(())
    ...
    async fn update_permissions(&self, path: &str, permissions: u8) -> Result<(), PytjaError> {
        sqlx::query("UPDATE file_nodes SET permissions = ? WHERE path = ?")
            .bind(permissions as i32).bind(path)
            .execute(&self.pool).await.map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
        Ok(())
    }

    async fn get_total_usage(&self, owner: &str) -> Result<usize, PytjaError> {
        let size: Option<i64> = sqlx::query_scalar("SELECT SUM(size) FROM file_nodes WHERE owner = ?")
            .bind(owner)
    ...
    async fn find_nodes(&self, pattern: &str) -> Result<Vec<String>, PytjaError> {
        let rows = sqlx::query("SELECT path FROM file_nodes WHERE name LIKE ?")
            .bind(pattern)
            .fetch_all(&self.pool).await.map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
    ...
    async fn get_all_files_content(&self) -> Result<Vec<(String, Vec<u8>)>, PytjaError> {
        let rows = sqlx::query("SELECT path, content FROM file_nodes WHERE is_folder = 0")
            .fetch_all(&self.pool).await.map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
    ...
    async fn get_audit_logs(&self, limit: u32, user_filter: Option<String>) -> Result<Vec<AuditLog>, PytjaError> {
        ...
        let result: Vec<AuditLog> = query.fetch_all(&self.pool).await
            .map_err(|e| PytjaError::DatabaseError(e.to_string()))?;
